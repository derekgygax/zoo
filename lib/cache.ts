...
export class GlobalCache {
  ...
  static setCache = (key: FORM_NAME, value: FormConfig, timeToLive: number = 600) => {
    const expiresAt = Date.now() + timeToLive * 1000;
    this.cache.set(key, { value, expiresAt });
  };

  static getCache = (key: FORM_NAME): FormConfig | undefined => {
  ...
  static deleteCache = (key: FORM_NAME) => {
    this.cache.delete(key);
  };
